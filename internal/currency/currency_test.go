...
func TestCurrencyList_Fetch(t *testing.T) {
	...
	t.Run("valid data", func(t *testing.T) {
		var cl CurrencyList
		cl.urlTemplate = srv.URL + "/valid%s"
		err := cl.Fetch("")
		if err != nil {
			t.Errorf("Can't parse data: %v", err)
			return
	...
	t.Run("invalid data", func(t *testing.T) {
		var cl CurrencyList
		cl.urlTemplate = srv.URL + "/invalid%s"
		err := cl.Fetch("")
		if err == nil {
			t.Errorf("Bad data, but no error!")
			return
...
func TestCurrencyList_SetCache(t *testing.T) {
	t.Run("main", func(t *testing.T) {
		var cCache CurrencyCache = CurrencyCache{
			CacheGet: func(date string) *string {
				var rv string = "testString"
				return &rv
			},
		...
		cl.SetCache(&cCache)
		if cl.cache == nil {
			t.Error("Can't set cache functions!")
			return
		}
		cacheData := cl.cache.CacheGet("")
		if cacheData == nil {
			t.Error("Cache function must return value, but got nil")
		}
		if *cacheData != "testString" {
			t.Errorf("Cache function must return \"testString\", but got: %v", cacheData)
		}
	})
}
